.PHONY: web-test install build clean bump-minor bump-patch bump-major index index-full setup fmt lint test test-heavy test-running-mark-reanalysis test-change-error-f5-retry test-idle-start-running test-orphaned-apply-index-locks check-scenario-set check pre-commit audit publish build-linux build-linux-x86 build-linux-arm
