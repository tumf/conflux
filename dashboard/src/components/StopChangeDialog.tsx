...
interface StopChangeDialogProps {
  isOpen: boolean;
  changeId: string;
  onConfirm: () => void;
  onCancel: () => void;
...
export function StopChangeDialog({
  isOpen,
  changeId,
  onConfirm,
  onCancel,
  isLoading,
}: StopChangeDialogProps) {
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-semibold text-[#fafafa]">Force Kill Change</h2>
        </div>

        <p className="mb-5 text-sm text-[#71717a]">
          Force-kill <span className="font-medium text-[#fafafa]">{changeId}</span>? This will
          terminate the in-flight execution immediately.
        </p>
