...
- To stop one runaway proposal without disturbing the rest, use `cflx client force-stop-change <change-id>` — or `cflx_control` with action `force_stop_change` and exactly one entry in `change_ids`. It is the only control that skips the graceful SIGTERM window: the owner SIGKILLs that proposal's managed process group, confirms it was reaped, then dequeues it and clears its execution mark. Unrelated changes and the process-wide run mode are untouched, completed worktree effects are preserved (`effects_rolled_back: false`), and the success outcome is `stopped`. It is never a way to stop everything — that is `cflx client force-stop`.
