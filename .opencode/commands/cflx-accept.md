...
**Required for current Conflux compatibility** — emit exactly ONE legacy
standalone plain-text verdict marker as the final machine-readable payload, on
...
This compatibility mode is required because some currently running Conflux
orchestrators still recognize only the legacy standalone marker contract.
Do NOT rely on JSON-only verdict output for this worktree until the runtime
rollout is complete.
...
- Any line that does not equal one of the legacy markers exactly (after
  stripping defensively-tolerated bold/italic/heading/blockquote/bullet
  prefixes) is rejected and treated as if no verdict was emitted, which
  forces acceptance to retry.
...
- NO code fences: wrapping the verdict marker in ``` blocks is WRONG
