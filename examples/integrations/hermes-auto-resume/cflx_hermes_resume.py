...
def extract_binding(result: object):
    ...
    # Hermes wraps MCP results as {"result": <text>, "structuredContent":
    # <object>}. Prefer the typed object, then the textual fallback. Only these
    # host-defined wrapper fields may cross the envelope boundary.
    if "schema_version" not in envelope:
        wrapped = envelope.get("structuredContent")
        if wrapped is None:
            wrapped = envelope.get("result")
        if wrapped is envelope:
            return None
        return extract_binding(wrapped)
...
def compose_message(event: dict) -> str:
    ...
    instance_id = _bounded(event.get("instance_id"), 200) or "unknown"

    lines = [
        AUTOMATION_MARKER,
        "",
        "event: %s" % event_type,
        "Conflux reported `%s` for change `%s` (execution `%s`, owner instance `%s`)."
        % (event_type, change_id, execution_id, instance_id),
        "",
