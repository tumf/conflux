...
impl ParallelExecutor {
    ...
    pub(super) async fn attempt_merge(
        ...
        // The archive worktree can be cleaned up before a deferred retry runs, so never
        // invoke `git status` from a missing archive path. Use the workspace-local
        ...
        for (change_id, archive_path) in change_ids.iter().zip(archive_paths.iter()) {
            ...
            match is_archive_commit_complete(change_id, Some(verification_root)).await {
                Ok(true) => {
...
#[cfg(test)]
mod tests {
    use super::{archive_completion_verification_root, DeferredMerge};
