...
// The conflict layer's own workspace double, shared with its unit tests.
use crate::parallel::conflict::fixtures::{ConflictProbe, MockWorkspaceManager};
...
use crate::parallel::resolve_state::{self, BatchState, SequentialMergeItem};

// The classifier's own in-memory doubles are shared with its unit tests so both
// layers exercise the same evidence shape.
use crate::parallel::resolve_state::fixtures::{
    archived, item, live, presynced_repo, with_complete_tasks, FakeEvidence, FakeRepo,
};
...
async fn prefinal_archive_evidence_comes_from_the_validated_worktree_head() {
    ...
    worktree_only
        ...
        .tree("a_tip", &[&archived("change-a")]);
    with_complete_tasks(&mut worktree_only, "a_tip", "change-a");
