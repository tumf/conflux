//! Common helpers for Git command execution.
//!
//! This module provides shared utilities for running Git commands,
//! similar to jj_commands.rs but for Git repositories.
...
pub async fn run_git<P: AsRef<Path>>(args: &[&str], cwd: P) -> Result<String> {
    let output = Command::new("git")
        ...
        .output()
        .await
        .map_err(|e| OrchestratorError::GitCommand(format!("Failed to execute git: {}", e)))?;
...
#[allow(dead_code)]
pub async fn run_git_silent<P: AsRef<Path>>(args: &[&str], cwd: P) -> Result<()> {
    let output = Command::new("git")
        .args(args)
        .current_dir(cwd.as_ref())
        .stdin(Stdio::null())
...
pub async fn check_git_repo<P: AsRef<Path>>(cwd: P) -> Result<bool> {
    // Check git --version
    let version_result = Command::new("git")
        .arg("--version")
        .current_dir(cwd.as_ref())
        .stdin(Stdio::null())
        .output()
...
pub async fn has_uncommitted_changes<P: AsRef<Path>>(cwd: P) -> Result<(bool, String)> {
...
pub async fn get_current_commit<P: AsRef<Path>>(cwd: P) -> Result<String> {
    run_git(&["rev-parse", "HEAD"], cwd).await
}

/// Get the current branch name.
/// Returns None if in detached HEAD state.
pub async fn get_current_branch<P: AsRef<Path>>(cwd: P) -> Result<Option<String>> {
...
pub async fn get_conflict_files<P: AsRef<Path>>(cwd: P) -> Result<Vec<String>> {
...
/// Remove a worktree.
pub async fn worktree_remove<P: AsRef<Path>>(cwd: P, worktree_path: &str) -> Result<()> {
...
/// Delete a branch.
pub async fn branch_delete<P: AsRef<Path>>(cwd: P, branch_name: &str) -> Result<()> {
...
/// Checkout a branch.
pub async fn checkout<P: AsRef<Path>>(cwd: P, branch_or_commit: &str) -> Result<()> {
...
pub async fn merge<P: AsRef<Path>>(cwd: P, branch_name: &str) -> Result<()> {
    let output = Command::new("git")
        ...
        .await
        .map_err(|e| {
            OrchestratorError::GitCommand(format!("Failed to execute git merge: {}", e))
        })?;
    ...
    if !output.status.success() {
        ...
        if combined.contains("CONFLICT") || combined.contains("Automatic merge failed") {
            return Err(OrchestratorError::GitConflict(combined.to_string()));
        }
...
#[allow(dead_code)]
pub async fn merge_abort<P: AsRef<Path>>(cwd: P) -> Result<()> {
    run_git(&["merge", "--abort"], cwd).await?;
    Ok(())
}

/// Stage all changes and commit with the given message.
#[allow(dead_code)] // Reserved for future use in commit operations
pub async fn add_and_commit<P: AsRef<Path>>(cwd: P, message: &str) -> Result<()> {
...
#[allow(dead_code)] // Reserved for future use in commit operations
pub async fn has_changes_to_commit<P: AsRef<Path>>(cwd: P) -> Result<bool> {
    let output = run_git(&["status", "--porcelain"], cwd).await?;
    Ok(!output.is_empty())
}
