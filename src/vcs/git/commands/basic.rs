...
/// Get the path to the .git directory for a repository.
///
/// Returns the absolute path to the .git directory (or the git directory for worktrees).
pub async fn get_git_dir<P: AsRef<Path>>(cwd: P) -> VcsResult<PathBuf> {
    let cwd_path = cwd.as_ref();
    let output = run_git(&["rev-parse", "--git-dir"], cwd_path).await?;
    ...
    if git_dir.is_absolute() {
        ...
        Ok(cwd_path.join(git_dir))
    }
}

/// Ensure the .git/info/exclude file contains the given pattern.
///
/// If the pattern does not exist in the file, it will be appended.
/// The pattern is treated as a literal string and checked for exact match (trimmed).
///
/// Returns Ok(true) if the pattern was added, Ok(false) if it already existed.
pub async fn ensure_git_info_exclude_pattern<P: AsRef<Path>>(
    cwd: P,
    pattern: &str,
) -> VcsResult<bool> {
    let git_dir = get_git_dir(&cwd).await?;
    let exclude_file = git_dir.join("info").join("exclude");

    debug!("Checking .git/info/exclude at {:?}", exclude_file);

    // Ensure the info directory exists
    let info_dir = git_dir.join("info");
    if !info_dir.exists() {
        fs::create_dir_all(&info_dir).await.map_err(|e| {
            VcsError::git_command(format!("Failed to create .git/info directory: {}", e))
        })?;
    }

    // Read existing content (or create empty file if it doesn't exist)
    let content = if exclude_file.exists() {
        fs::read_to_string(&exclude_file).await.map_err(|e| {
            VcsError::git_command(format!("Failed to read .git/info/exclude: {}", e))
        })?
    } else {
        String::new()
    };

    // Check if pattern already exists (exact match on trimmed lines)
    let pattern_trimmed = pattern.trim();
    if content.lines().any(|line| line.trim() == pattern_trimmed) {
        debug!("Pattern '{}' already exists in .git/info/exclude", pattern);
        return Ok(false);
    }
    ...
    new_content.push_str(pattern_trimmed);
    new_content.push('\n');

    fs::write(&exclude_file, new_content)
        .await
        .map_err(|e| VcsError::git_command(format!("Failed to write .git/info/exclude: {}", e)))?;

    debug!("Added pattern '{}' to .git/info/exclude", pattern);
    Ok(true)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_ensure_git_info_exclude_pattern_adds_new() {
        let temp_dir = TempDir::new().unwrap();
        ...
        // Add pattern
        let added = ensure_git_info_exclude_pattern(temp_dir.path(), "openspec/changes/*/approved")
            .await
            .unwrap();
        assert!(added, "Pattern should be added on first call");

        // Verify pattern exists
        let git_dir = get_git_dir(temp_dir.path()).await.unwrap();
        let exclude_file = git_dir.join("info").join("exclude");
        assert!(exclude_file.exists());

        let content = tokio::fs::read_to_string(&exclude_file).await.unwrap();
        assert!(content.contains("openspec/changes/*/approved"));
        assert_eq!(
            content
                .lines()
                .filter(|l| l.trim() == "openspec/changes/*/approved")
                .count(),
            1,
    ...
    #[tokio::test]
    async fn test_ensure_git_info_exclude_pattern_idempotent() {
        let temp_dir = TempDir::new().unwrap();
        ...
        let added1 =
            ensure_git_info_exclude_pattern(temp_dir.path(), "openspec/changes/*/approved")
                .await
                .unwrap();
        assert!(added1, "Pattern should be added on first call");

        // Add pattern second time - should not be added again
        let added2 =
            ensure_git_info_exclude_pattern(temp_dir.path(), "openspec/changes/*/approved")
                .await
                .unwrap();
        assert!(!added2, "Pattern should not be added on second call");

        // Verify pattern exists only once
        let git_dir = get_git_dir(temp_dir.path()).await.unwrap();
        let exclude_file = git_dir.join("info").join("exclude");
        let content = tokio::fs::read_to_string(&exclude_file).await.unwrap();
        assert_eq!(
            content
                ...
                .count(),
            1,
            "Pattern should appear exactly once after multiple calls"
        );
    }

    #[tokio::test]
    async fn test_ensure_git_info_exclude_pattern_excludes_untracked() {
        let temp_dir = TempDir::new().unwrap();
        ...
        // Create approved file that should be excluded
        let approved_dir = temp_dir.path().join("openspec/changes/test-change");
        std::fs::create_dir_all(&approved_dir).unwrap();
        std::fs::write(approved_dir.join("approved"), "approved").unwrap();

        // Check status before adding pattern - should show openspec directory
        let (has_changes_before, status_before) =
            has_uncommitted_changes(temp_dir.path()).await.unwrap();
        assert!(
            has_changes_before,
            "Openspec directory should be untracked initially"
        );
        assert!(
            status_before.contains("openspec"),
            "Status should mention openspec directory"
        );

        // Add pattern to .git/info/exclude
        ensure_git_info_exclude_pattern(temp_dir.path(), "openspec/changes/*/approved")
            .await
            .unwrap();

        // Check status after adding pattern - should NOT show openspec directory
        let (has_changes_after, status_after) =
            has_uncommitted_changes(temp_dir.path()).await.unwrap();
        assert!(
            !has_changes_after,
            "Openspec/approved pattern should be excluded after pattern added"
        );
        assert!(
            !status_after.contains("openspec"),
            "Status should not mention openspec directory"
        );
    }
}
