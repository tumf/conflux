...
mod tests {
    ...
    fn test_vcs_error_constructors() {
        ...
        assert!(matches!(err, VcsError::Command { backend: VcsBackend::Jj, .. }));

        let err = VcsError::git_command("test error");
        assert!(matches!(err, VcsError::Command { backend: VcsBackend::Git, .. }));

        let err = VcsError::jj_conflict("conflict details");
        assert!(matches!(err, VcsError::Conflict { backend: VcsBackend::Jj, .. }));

        let err = VcsError::git_conflict("conflict details");
        assert!(matches!(err, VcsError::Conflict { backend: VcsBackend::Git, .. }));
    }
}
