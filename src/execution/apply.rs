...
pub async fn create_progress_commit<W: WorkspaceManager + ?Sized>(
    ...
    workspace_manager.snapshot_working_copy(workspace_path).await?;
...
pub async fn create_final_commit<W: WorkspaceManager + ?Sized>(
    ...
    debug!("Creating final commit for {}: {}", change_id, commit_message);
...
pub async fn get_workspace_revision<W: WorkspaceManager + ?Sized>(
    ...
    workspace_manager.get_revision_in_workspace(workspace_path).await
}
