...
use crate::vcs::{GitWorkspaceManager, JjWorkspaceManager, VcsBackend, Workspace, WorkspaceManager, WorkspaceStatus};
...
impl ParallelExecutor {
    ...
    pub async fn check_vcs_available(&self) -> Result<bool> {
        self.workspace_manager.check_available().await.map_err(Into::into)
    }
    ...
    async fn execute_group(&mut self, group: &ParallelGroup) -> Result<()> {
        ...
        let base_revision = self.workspace_manager.get_current_revision().await.map_err(OrchestratorError::from)?;
        info!(
    ...
    async fn merge_and_resolve(&self, revisions: &[String]) -> Result<()> {
        ...
        match merge_result {
            ...
            Err(VcsError::Conflict { details, .. }) => {
                ...
                self.resolve_conflicts_with_retry(revisions, &details)
                    .await
            }
            Err(e) => Err(e.into()),
        }
    ...
    async fn detect_conflicts(&self) -> Result<Vec<String>> {
        self.workspace_manager.detect_conflicts().await.map_err(OrchestratorError::from)
    }

    /// Get VCS status output for context
    async fn get_vcs_status(&self) -> Result<String> {
        self.workspace_manager.get_status().await.map_err(OrchestratorError::from)
    }
    ...
    pub async fn cleanup(&mut self) -> Result<()> {
        self.workspace_manager.cleanup_all().await.map_err(OrchestratorError::from)
    }
}
