        ...
        match event {
            ...
            OrchestratorEvent::ProcessingCompleted(id) => {
                if let Some(change) = self.changes.iter_mut().find(|c| c.id == id) {
                    ...
                    if let Ok(progress) =
                        task_parser::parse_progress_with_fallback(&id, worktree_path)
                    {
            ...
            OrchestratorEvent::ArchiveStarted(id) => {
                if let Some(change) = self.changes.iter_mut().find(|c| c.id == id) {
                    ...
                    // Use unified fallback helper to read from worktree → archive → base
                    let worktree_path = self.worktree_paths.get(&id).map(|p| p.as_path());
...
fn test_explicit_queue_addition_in_stopped_mode_works() {
    // Scenario: Execution mark toggle in Stopped mode
    // WHEN TUI is in Stopped mode
    // AND user presses Space on a not-queued change
    // THEN the execution mark (selected) should toggle
    // AND queue_status should remain NotQueued (will become Queued on F5 resume)
    ...
    // Initial state: NotQueued, not selected
    app.changes[0].queue_status = QueueStatus::NotQueued;
    ...
    // Should NOT return a command (just state change)
    assert!(cmd.is_none());
...
fn test_stopped_event_cleans_up_processing_changes() {
    ...
    // Simulate processing state for multiple changes with execution marks
    app.changes[0].queue_status = QueueStatus::Processing;
    app.changes[0].started_at = Some(Instant::now());
    ...
    // All should be reset to NotQueued (not Queued)
    assert!(matches!(
...
// === Tests for fix-tui-progress-fallback ===

#[test]
fn test_archive_started_uses_worktree_fallback() {
    use tempfile::TempDir;

    let temp_dir = TempDir::new().unwrap();
    let worktree_path = temp_dir.path();

    // Create worktree structure with tasks.md
    ...
    let changes = vec![create_approved_change("test-change", 0, 0)];
    let mut app = AppState::new(changes);
    ...
    // Progress should be read from worktree
...
#[test]
fn test_change_archived_uses_worktree_fallback() {
    use tempfile::TempDir;

    let temp_dir = TempDir::new().unwrap();
    let worktree_path = temp_dir.path();

    // Create worktree archive structure with tasks.md
    ...
    let changes = vec![create_approved_change("test-archived", 0, 0)];
    let mut app = AppState::new(changes);
    app.changes[0].started_at = Some(Instant::now());
    ...
    // Progress should be read from worktree archive
...
#[test]
fn test_merge_completed_uses_worktree_fallback() {
    use tempfile::TempDir;
    ...
    let changes = vec![create_approved_change("test-merged", 0, 0)];
    let mut app = AppState::new(changes);
    app.changes[0].started_at = Some(Instant::now());
    ...
    // Progress should be read from worktree archive
    assert_eq!(app.changes[0].completed_tasks, 2);
    assert_eq!(app.changes[0].total_tasks, 2);
    assert_eq!(app.changes[0].queue_status, QueueStatus::Merged);
}

#[test]
fn test_progress_preserved_when_worktree_fallback_fails() {
    let changes = vec![create_approved_change("test-change", 5, 7)];
    let mut app = AppState::new(changes);
    app.changes[0].queue_status = QueueStatus::Archiving;

    // No worktree path set (fallback will fail)

    // Trigger ArchiveStarted event with no worktree
    app.handle_orchestrator_event(OrchestratorEvent::ArchiveStarted("test-change".to_string()));
...
/// Test that MergeWait is auto-cleared when worktree doesn't exist
#[test]
fn test_merge_wait_auto_cleared_when_worktree_missing() {
    use std::collections::HashSet;
    ...
    // WHEN: auto_clear_merge_wait is called with worktree_change_ids NOT containing the change
    let worktree_change_ids = HashSet::new(); // change-a has no worktree
    let worktree_not_ahead_ids = HashSet::new();
    app.auto_clear_merge_wait(&worktree_change_ids, &worktree_not_ahead_ids);

    // THEN: MergeWait is cleared to Queued
    assert_eq!(
        app.changes[0].queue_status,
        QueueStatus::Queued,
        "MergeWait should be auto-cleared to Queued when worktree is missing"
    );
    ...
    assert!(app
        ...
        .unwrap()
        .message
        .contains("Auto-cleared MergeWait"));
    assert!(app
        .logs
        .last()
        .unwrap()
...
/// Test that MergeWait is auto-cleared when worktree exists but not ahead of base
#[test]
fn test_merge_wait_auto_cleared_when_not_ahead() {
    use std::collections::HashSet;
    ...
    // WHEN: auto_clear_merge_wait is called with worktree existing but not ahead
    let mut worktree_change_ids = HashSet::new();
    worktree_change_ids.insert("change-a".to_string()); // Worktree exists
    let mut worktree_not_ahead_ids = HashSet::new();
    worktree_not_ahead_ids.insert("change-a".to_string()); // But not ahead
    app.auto_clear_merge_wait(&worktree_change_ids, &worktree_not_ahead_ids);
    ...
    assert_eq!(
        app.changes[0].queue_status,
        QueueStatus::Queued,
        "MergeWait should be auto-cleared to Queued when worktree is not ahead"
    );

    // AND: A log entry is added with "merged to base" reason
    assert!(!app.logs.is_empty());
    assert!(app
        .logs
        .last()
    ...
    assert!(app
        ...
        .unwrap()
        .message
        .contains("worktree merged to base"));
}

/// Test that MergeWait is preserved when worktree exists and is ahead
#[test]
fn test_merge_wait_preserved_when_ahead() {
    use std::collections::HashSet;
    ...
    // WHEN: auto_clear_merge_wait is called with worktree existing and ahead
    let mut worktree_change_ids = HashSet::new();
    worktree_change_ids.insert("change-a".to_string()); // Worktree exists
    let worktree_not_ahead_ids = HashSet::new(); // And is ahead (not in not_ahead set)
    let initial_log_count = app.logs.len();
    app.auto_clear_merge_wait(&worktree_change_ids, &worktree_not_ahead_ids);

    // THEN: MergeWait is preserved
    assert_eq!(
        app.changes[0].queue_status,
        QueueStatus::MergeWait,
        "MergeWait should be preserved when worktree exists and is ahead"
    );

    // AND: No log entry is added (no change occurred)
    assert_eq!(
        app.logs.len(),
        initial_log_count,
        "No new log entries should be added when MergeWait is preserved"
    );
}
