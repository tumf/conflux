...
use super::terminal_helpers::{execute_worktree_command, suspend_terminal_and_execute_sync};
...
pub async fn handle_plus_key(ctx: &mut KeyEventContext<'_>) -> Result<()> {
    ...
    if !super::worktree_helpers::should_trigger_worktree_command(ctx.config, is_git_repo) {
        return Ok(());
    }
    ...
    let worktree_path = super::worktree_helpers::build_worktree_path(ctx.worktree_base_dir);
