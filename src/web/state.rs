...
fn apply_reducer_derived_operator_state(
    ...
    for change in &mut state.changes {
        ...
        change.blocker = runtime
            ...
            .map(project_blocker);
        // The typed Apply iteration-limit diagnostic reaches a client through
        // this sanitized detail — attempts, ceiling, and the last actionable
        // failure — rather than through an action gate of its own. Retry
        // eligibility is classified from the row's terminal-error evidence, so
