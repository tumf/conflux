...
コマンド:
  ...
  init             新しい設定ファイルを初期化
  check-conflicts  変更間のスペックデルタファイルのコンフリクトをチェック

オプション:
  -c, --config <CONFIG>   カスタム設定ファイルのパス（JSONC形式）
      --web               Web監視サーバーを有効化
      --web-port <PORT>   Web監視サーバーのポート（デフォルト: 0 = OSが自動割当）
      --web-bind <ADDR>   Web監視サーバーのバインドアドレス（デフォルト: 127.0.0.1）
  -h, --help              ヘルプを表示
  -V, --version           バージョンを表示
```
...
```
オプション:
      --change <CHANGE>        処理する変更を指定（カンマ区切り）
  -c, --config <CONFIG>        カスタム設定ファイルのパス（JSONC形式）
      --max-iterations <N>     最大イテレーション数（0 = 制限なし）
      --parallel               Gitworktreesを使った並列実行モードを有効化
      --max-concurrent <N>     最大同時ワークスペース数（デフォルト: 3）
      --dry-run                実行せずに並列化グループをプレビュー
      --vcs <VCS>              VCSバックエンド: auto または git（デフォルト: auto）
      --no-resume              ワークスペースの自動レジュームを無効化
      --web                    Web監視サーバーを有効化
...
TUI（デフォルトモード）はWeb監視オプションもサポートします:
...
```
src/
├── main.rs                  # エントリーポイント（デフォルト: TUIモード）
├── cli.rs                   # CLIの引数解析
├── config/                  # 設定ファイル解析（JSONC）
├── orchestrator.rs          # メインのオーケストレーションループ
├── hooks.rs                 # ライフサイクルフックの実行
├── task_parser.rs           # タスクファイルの解析と進捗計算
├── templates.rs             # 設定テンプレート（claude、opencode、codex）
├── tui/                     # インタラクティブTUIダッシュボード（ratatui）
├── parallel/                # Gitworktreesを使った並列実行
├── web/                     # Web監視サーバー
└── ...                      # その他のモジュール
```
...
ビルド手順、テスト、フルプロジェクト構造については [Development Guide](docs/guides/DEVELOPMENT.md) を参照してください。
