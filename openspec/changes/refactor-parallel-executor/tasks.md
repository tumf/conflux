...
- [ ] 1.2 Update `src/tui/orchestrator.rs` to use `ParallelEventBridge::convert()`
- [ ] 1.3 Remove old inline match block from orchestrator

## 2. Add WorkspaceCleanupGuard
- [ ] 2.1 Implement `WorkspaceCleanupGuard` struct in `src/parallel_executor.rs`
- [ ] 2.2 Integrate guard into `execute_group()` method

## 3. Extract Apply Loop Helpers
- [ ] 3.1 Extract `check_task_progress()` helper function
- [ ] 3.2 Extract `summarize_output()` helper function
- [ ] 3.3 Refactor `execute_apply_in_workspace()` to use helpers
