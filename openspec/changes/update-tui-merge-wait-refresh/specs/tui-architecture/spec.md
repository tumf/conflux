...
`ResolveWait`は、ユーザーが`M`キーでmerge resolveを開始した直後から`ResolveStarted`を受信するまでの単一起動待ち状態であり、Space/@によるキュー操作でDynamicQueueを変更してはならない（MUST NOT）。TUIは`ResolveWait`を`resolve wait`として表示し、操作対象外であることが明確でなければならない（MUST）。
