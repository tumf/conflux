...
- [x] 仕様 `openspec/changes/add-remote-tui-client/specs/tui-architecture/spec.md:20-25` の「iteration 非後退ルール」が実フローで未実装です。`src/tui/runner.rs:379-383` の `RemoteChangeUpdate` 送信と `src/events.rs:421-427` のイベント定義には `iteration_number` がなく、`src/tui/state.rs:1326-1338` でも進捗値のみ更新しています。`RemoteChangeUpdate` に `iteration_number` を追加して `AppState::handle_orchestrator_event` で `update_iteration_monotonic` を適用し、`iteration=3` 表示中に `iteration=2` 更新を受けても後退しない unit test を追加してください。（修正: `src/events.rs` の `RemoteChangeUpdate` に `iteration_number: Option<u32>` を追加。`src/tui/runner.rs` で `change.iteration_number` を送信。`src/tui/state.rs` の `RemoteChangeUpdate` ハンドラで `change.update_iteration_monotonic(iteration_number)` を呼ぶよう修正。`test_remote_change_update_iteration_non_regression_rule`、`test_remote_change_update_iteration_increases`、`test_remote_change_update_iteration_none_no_op` の 3 テストを追加し、`iteration=3` 表示中に `iteration=2` 更新を受けても後退しないことを検証。`cargo build`/`cargo test`/`cargo clippy -- -D warnings`/`cargo fmt --check` すべてパス。）
