...
- [x] `src/tui/state/event_handlers/completion.rs:92-100` は queued resolve の follow-up で `"Auto-starting resolve for '{}' from queue"` とログしつつ `Some(TuiCommand::ResolveMerge(next_change_id))` を返しています。しかし `src/tui/command_handlers.rs:562-576` の `ResolveMerge` 処理は実行開始ではなく reducer-owned retry intent の記録と scheduler wakeup だけです。このため、この completion 経路の wording だけが依然として『今すぐ resolve を開始した』と読め、`ResolveStarted` event だけが開始を表すという canonical rule と不整合です。queued follow-up 側も scheduler retry intent を表す文言へ揃える必要があります。
