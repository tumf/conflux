...
- [x] Run required Rust checks. Completion condition: formatting and targeted tests pass, and any test taking over 1 second is either optimized or marked heavy per repository policy. (verification: integration - `cargo fmt --check`; targeted `cargo test` for affected parallel/TUI modules)
