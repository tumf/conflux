...
## Purpose
コマンド実行キューの機能を定義し、時間差起動と自動リトライによるコマンド実行の安定性向上を実現する。
...
#### Scenario: リトライ可能エラーでの自動再実行

- **GIVEN** リトライパターンに `Cannot find module` が設定されている
- **AND** 最大リトライ回数が2回に設定されている
- **WHEN** コマンド実行が `Cannot find module '../../lib/buffer_utils.js'` エラーで失敗
- **THEN** リトライ可能と判定される
- **AND** 設定された待機時間後に自動的に再実行される

#### Scenario: 最大リトライ回数到達時のエラー返却

- **GIVEN** 最大リトライ回数が2回に設定されている
- **WHEN** コマンド実行が3回連続でリトライ可能エラーで失敗
- **THEN** 1回目と2回目はリトライされる
- **AND** 3回目の失敗後、エラーが返却される（リトライしない）

#### Scenario: リトライ不可能エラーでの即座終了

- **GIVEN** リトライパターンに `Cannot find module` のみ設定されている
- **WHEN** コマンド実行が `Syntax error` で失敗
- **THEN** リトライ不可能と判定される
- **AND** 即座にエラーが返却される（リトライしない）

#### Scenario: リトライ成功時の正常終了

- **GIVEN** 最大リトライ回数が2回に設定されている
- **WHEN** 1回目のコマンド実行がリトライ可能エラーで失敗
- **AND** 2回目のコマンド実行が成功（終了ステータス0）
- **THEN** 正常終了として完了する
