...
- [x] 現在の config merge contract を characterization test で固定する（verification: unit - custom/project/XDG env/XDG default/platform/default の優先順、`Some` 上書き、`None` 非上書きを確認する）
- [x] hooks deep merge と通常 Option 上書きの違いを characterization test で固定する（verification: unit - hooks の個別フィールド merge と server/proposal session 等の上書き挙動を確認する）
- [x] deprecated path helper の後方互換を characterization test で固定する（verification: unit - `get_global_config_path` と `get_xdg_config_path` の既存 fallback/precedence を確認する）
