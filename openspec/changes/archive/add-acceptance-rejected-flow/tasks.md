...
- [x] 5. `src/orchestration/rejection.rs` (新規): rejection フロー関数を実装する — REJECTED.md 生成、base チェックアウト、コミット、`openspec resolve` 呼び出し、worktree 削除 (verification: ユニットテスト)
...
- [x] 15. 統合テスト: Blocked → Rejected フロー全体の e2e テストを追加する (verification: `cargo test --test e2e_tests`)
