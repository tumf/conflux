## 1. 実装
- [x] 1.1 `default_workspace_base_dir()` のデフォルトパスを `cflx` に統一し、macOS のフォールバックを `~/.local/share` に変更する
...
- [x] 1.2 `default_workspace_base_dir()` のテストとコメントを新しいパスに合わせて更新する
  - 検証: `src/config/defaults.rs` 内のテストが `cflx` と新しい macOS フォールバックを期待するよう更新されていることを確認する

## 2. 検証
- [x] 2.1 既存テストが通ることを確認する
