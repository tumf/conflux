...
- [x] 4. 起動ログの典型経路を検証し、`cflx run` / `cflx server` の startup log から version/build と mode が読み取れることを確認する (verification: integration - `src/main.rs` に `log_startup("run")` / `log_startup("server")` 呼び出しが存在し、`src/main.rs` の `log_startup` helper が `env!("CARGO_PKG_VERSION")` と build metadata を使って `Starting cflx ... mode=...` を出力することを確認)
