...
### 1.1 型定義の追加
- [x] `src/tui/types.rs`: `ViewMode` enum追加 (Changes/Worktrees)
- [x] `src/tui/types.rs`: `WorktreeInfo` struct追加
...
- [x] `src/tui/types.rs`: `WorktreeAction` enum追加 (Delete/OpenEditor/OpenShell)
- [x] `src/tui/types.rs`: `MergeConflictInfo` struct追加
- [x] 単体テスト: 各型のメソッドとロジックをテスト

**検証**: `cargo test types` がパス ✓

### 1.2 Git worktree操作の実装
- [x] `src/vcs/git/commands.rs`: `list_worktrees()` 関数実装
  - `git worktree list --porcelain` 実行
  - Porcelain形式のパーサー実装 (worktree/HEAD/branch/detached)
  - 最初のworktreeを `is_main=true` として扱う
- [x] `src/vcs/git/commands.rs`: `worktree_remove()` 関数実装 (既存)
  - `git worktree remove <path>` 実行
- [x] `src/vcs/git/commands.rs`: `is_working_directory_clean()` 関数実装
  - `git status --porcelain` でチェック
- [x] `src/vcs/git/commands.rs`: `get_current_branch()` 関数実装 (既存)
  - `git branch --show-current` 実行
- [x] 統合テスト: 実際のgit repositoryを使用したテスト

**検証**: `cargo test vcs::git::commands` がパス ✓

### 1.3 AppStateの拡張
- [x] `src/tui/state/mod.rs`: Worktree関連フィールド追加
  - `view_mode`, `worktrees`, `worktree_cursor_index`, `worktree_list_state`, `pending_worktree_action`
- [x] `src/tui/state/mod.rs`: Worktreeナビゲーションメソッド実装
  - `worktree_cursor_up()`, `worktree_cursor_down()`
- [x] `src/tui/state/mod.rs`: Worktree削除リクエスト実装
  - `request_worktree_delete_from_list()` - バリデーション含む
  - Main worktreeチェック、Processing中チェック
- [x] `src/tui/state/mod.rs`: ユーティリティメソッド実装
  - `get_selected_worktree_path()`, `get_selected_worktree()`
  - `confirm_worktree_action_delete()`, `cancel_worktree_action()`
- [x] 単体テスト: カーソル移動、削除バリデーション

**検証**: `cargo test tui::state` がパス ✓

### 1.4 イベント定義
- [x] `src/tui/events.rs`: `TuiCommand` に追加
  - `DeleteWorktreeByPath(PathBuf)`
- [x] `src/tui/events.rs`: `ExecutionEvent` に追加
  - `WorktreesRefreshed { worktrees: Vec<WorktreeInfo> }`

**検証**: コンパイルエラーがないこと ✓

### 1.5 Worktreeビューのレンダリング
- [x] `src/tui/render.rs`: `render()` 関数を修正
  - `view_mode` に基づいてview切り替え
- [x] `src/tui/render.rs`: `render_worktree_view()` 実装
  - Header/List/Footer のレイアウト
- [x] `src/tui/render.rs`: `render_worktree_list()` 実装
  - Worktree情報の表示 (label, branch)
  - カーソルとハイライト
  - 動的キーヒント (基本版: Tab, D, e, Enter, +)
- [x] `src/tui/render.rs`: `render_footer_worktree()` 実装
  - Worktree数、警告メッセージ表示
- [x] `src/tui/render.rs`: Changesビューのキーヒント修正
  - `D` と `+` を削除、`Tab: worktrees` を追加

**検証**: TUIを起動してWorktreeビューが表示されること (データなしでもOK) ✓

### 1.6 キーバインドの実装
- [x] `src/tui/runner.rs`: Tabキーでview切り替え実装
  - Changes → Worktrees (worktreeリスト読み込み)
  - Worktrees → Changes
- [x] `src/tui/runner.rs`: Worktreeビュー用のカーソル操作
  - ↑↓/jk キーをview依存で処理
- [x] `src/tui/runner.rs`: Dキー (Worktree削除) 実装
  - Worktreeビューでのみ動作
  - 確認ダイアログ表示
- [x] `src/tui/runner.rs`: eキー (エディタ) を両view対応に修正
  - Changes: 既存動作維持
  - Worktrees: `launch_editor_in_dir()` 使用
- [x] `src/tui/runner.rs`: Enterキー (シェル) 実装
  - Worktreeビューでworktree_command実行
  - 設定なしの場合は無効化
- [x] `src/tui/runner.rs`: +キー (Worktree作成) をWorktreeビューに移動
  - ws-session-{timestamp} 命名に変更
  - ブランチ作成 (detachedではなく)
- [x] `src/tui/runner.rs`: Changesビューから D と + キーハンドラを削除
- [x] `src/tui/utils.rs`: `launch_editor_in_dir()` 実装
  - 指定ディレクトリでエディタを起動

**検証**: 各キーバインドが期待通り動作すること、Changesビューで D/+ が無効になること ✓

### 1.7 コマンド処理
- [x] `src/tui/runner.rs`: `DeleteWorktreeByPath` コマンドハンドラ実装
  - `worktree_remove()` 呼び出し
  - 成功時にworktreeリスト更新
  - エラーログ表示
- [x] `src/tui/state/events.rs`: `WorktreesRefreshed` イベントハンドラ実装
  - worktreeリスト更新
  - カーソル範囲外チェック
...
### 1.8 Phase 1 統合テスト
- [x] E2Eテスト: Tabキーでview切り替え
- [x] E2Eテスト: Worktree作成 → リスト表示 → 削除
- [x] E2Eテスト: エディタ・シェル起動
...
### 2.1 コンフリクト検出の実装
- [x] `src/vcs/git/commands.rs`: `check_merge_conflicts()` 関数実装
  - `git merge --no-commit --no-ff <branch>` でテストマージ
  - コンフリクト検出 (stderr解析)
  - `git merge --abort` で元に戻す
  - `parse_conflict_files()` でファイル名抽出
- [x] 単体テスト: コンフリクトパーサーのテスト
- [x] 統合テスト: 実際のgit repositoryでコンフリクト検出

**検証**: コンフリクトありのブランチを正しく検出できること ✓

### 2.2 ブランチマージの実装
- [x] `src/vcs/git/commands.rs`: `merge_branch()` 関数実装
  - ワーキングディレクトリクリーンチェック
  - `git merge --no-ff --no-edit <branch>` 実行
  - コンフリクト時に `git merge --abort`
  - エラーメッセージの解析と返却
- [x] 統合テスト: 正常マージとコンフリクトマージ

**検証**: コンフリクトなしのマージが成功すること、コンフリクトありは中断されること ✓

### 2.3 マージイベントの追加
- [x] `src/tui/events.rs`: `TuiCommand::MergeWorktreeBranch` 追加
  - `worktree_path`, `branch_name` フィールド
- [x] `src/tui/events.rs`: マージ関連イベント追加
...
### 2.4 マージリクエストの実装
- [x] `src/tui/state/mod.rs`: `request_merge_worktree_branch()` 実装
  - Main worktreeチェック
  - Detached HEADチェック
  - コンフリクトありチェック
...
### 2.5 マージイベントハンドリング
- [x] `src/tui/state/events.rs`: マージイベントハンドラ実装
  - `MergeStarted`: ログ表示
...
### 2.6 マージキーバインド
- [x] `src/tui/runner.rs`: Mキー (Shift+M) ハンドラ実装
  - Worktreeビューでのみ動作
  - `request_merge_worktree_branch()` 呼び出し
- [x] `src/tui/runner.rs`: `MergeWorktreeBranch` コマンドハンドラ実装
  - 非同期タスクでマージ実行
...
### 2.7 Phase 2 統合テスト
- [x] E2Eテスト: コンフリクトなしマージ
- [x] E2Eテスト: コンフリクトあり時にマージ不可
- [x] E2Eテスト: エラー時のポップアップ表示
...
- [x] `src/tui/runner.rs`: `load_worktrees_with_conflict_check()` 実装
  - `list_worktrees()` 実行
  - 各worktreeについてコンフリクトチェック
  - Main/Detached はスキップ
  - チェック失敗時は `merge_conflict = None`
- [x] Tabキー切り替え時に `load_worktrees_with_conflict_check()` 使用

**検証**: Worktreeビュー表示時にコンフリクトチェックが実行されること ✓

### 3.2 並列コンフリクトチェック
- [x] `src/tui/runner.rs`: 並列チェック実装
  - `JoinSet` で複数worktreeを同時チェック
  - エラーハンドリング
- [x] パフォーマンステスト: 4つのworktreeで1秒未満

**検証**: 並列実行が正常動作すること、パフォーマンス要件を満たすこと ✓

### 3.3 UIにコンフリクト表示
- [x] `src/tui/render.rs`: `render_worktree_list()` にコンフリクトバッジ追加
  - `⚠{count}` 形式 (赤色、太字)
  - コンフリクトありの行を赤色でハイライト
- [x] `src/tui/render.rs`: Mキーヒントの条件付き表示
  - コンフリクトなし且つブランチありの場合のみ表示

**検証**: コンフリクトありのworktreeに警告マークが表示されること、Mキーが無効化されること ✓

### 3.4 自動リフレッシュ統合
- [x] `src/tui/runner.rs`: Refresh taskでworktreeチェック追加
...
### 3.6 ドキュメント更新
- [x] AGENTS.md: Worktreeビュー機能の説明追加
- [x] AGENTS.md: マージ機能とコンフリクト検出の説明
- [x] AGENTS.md: トラブルシューティングセクション
- [x] コード内コメント: 複雑なロジックに説明追加
...
**完了条件**: 全自動テストがパス、既存機能に影響なし ✓

---

## Future work

TUIを起動して全機能の手動確認 (requires human interaction)
  - View切り替え
  - Worktree作成・削除
  - エディタ・シェル起動
  - マージ (コンフリクトなし)
