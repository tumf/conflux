...
Add an `init` subcommand that generates a `.openspec-orchestrator.jsonc` configuration template file. The command supports a `--template` flag to select from predefined agent configurations (opencode, claude, codex), with claude as the default.
...
- Generate `.openspec-orchestrator.jsonc` with appropriate commands and hooks
