...
分析結果のバリデーションは、`dependencies` 内の依存先 ID が `order` または in-flight 変更 ID のいずれかに含まれていれば有効としなければならない（MUST）。`order` にも in-flight にも含まれない依存先 ID はバリデーションエラーとしなければならない（MUST）。
...
#### Scenario: Validate in-flight dependency references
- **GIVEN** 分析結果の `dependencies` に in-flight 変更 ID への参照が含まれる
- **AND** その ID は `order` には含まれないが in-flight 一覧に含まれる
- **WHEN** バリデーションが実行される
- **THEN** バリデーションは成功する

#### Scenario: Reject unknown dependency references
- **GIVEN** 分析結果の `dependencies` に `order` にも in-flight にも含まれない ID への参照がある
- **WHEN** バリデーションが実行される
- **THEN** バリデーションはエラーを返す
