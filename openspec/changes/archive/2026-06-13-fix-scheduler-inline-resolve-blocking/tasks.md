...
- [x] Task 1: Make `attempt_merge()` lane entry non-blocking in `src/parallel/merge.rs`:
...
- [x] Task 2: Extend the scheduler merge-result channel payload
...
- [x] Task 3: Convert `maybe_dispatch_resolve_wait_retry()` in
...
- [x] Task 4: Route the inline `retry_deferred_base_lane_waiters().await` call sites in
...
- [x] Task 5: Include the spawned retry window in drain/idle accounting: cover it with
      ...
      result arrives
- [x] Task 6: Add regression tests with a slow/gated resolve command in
      ...
      green on new code
- [x] Task 7: Verify deferred-then-retried recovery still converges: a merge deferred by
...
- [x] Task 8: Run quality gates with `cargo fmt --check`, `cargo clippy`, and the parallel
...
- [x] src/parallel/queue_state.rs::spawn_base_lane_retry_task lines 956-989 unconditionally sends Ok(MergeTaskOutcome::Merged) after retry_deferred_merges_for / retry_deferred_rejection_review_for return, so spawned ResolveWait/RejectWait retry tasks do not report the real merged/deferred/failed outcome through MergeResult. This violates openspec/changes/fix-scheduler-inline-resolve-blocking/specs/parallel-execution/spec.md lines 9-11 and tasks.md Task 2; fix by returning and forwarding the actual retry outcome so Deferred re-enters the scheduler retry flow instead of being treated as success.
  verification: integration - cargo test -p conflux parallel::tests::auto_resolve and parallel::tests::manual_resolve pass on the new code, confirming the retry outcome is reported correctly.
