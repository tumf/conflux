...
- [x] Extract `handle_post_analysis_capacity(...)` that performs post-analysis slot recalculation and capacity-zero suppression. (verification: integration - `cargo test parallel::tests::auto_resolve::test_auto_resolve_zero_capacity_runs_analysis_but_suppresses_apply_dispatch` and `cargo test parallel::tests::manual_resolve::test_manual_resolve_zero_capacity_runs_analysis_but_suppresses_apply_dispatch` cover zero-capacity dispatch suppression)
- [x] Refactor `perform_reanalysis_and_dispatch` to call the extracted helpers, reducing body to ≤80 non-comment lines. (verification: manual - `src/parallel/queue_state.rs` contains `ReanalysisDispatchContext` and `perform_reanalysis_and_dispatch`, and `cargo clippy -- -D warnings` reports no `too_many_arguments` allow on that function)
- [x] Add spec delta under `openspec/changes/extract-reanalysis-dispatch-guards/specs/parallel-execution/spec.md`. (verification: manual - source path `openspec/changes/extract-reanalysis-dispatch-guards/specs/parallel-execution/spec.md`; runnable command `cflx openspec validate extract-reanalysis-dispatch-guards --strict`)
