...
- [x] Convert the historical Acceptance #2 follow-up from a self-referential final-validation checkbox into non-checkbox narrative while preserving its concrete evidence, so `tasks.md` no longer asks the archive gate to validate itself as task evidence. (verification: manual - repository evidence is `openspec/changes/fix-error-retry-gating/tasks.md` lines 29-35, where the Acceptance #2 history is narrative-only with no checklist item and no self-referential `cflx openspec validate ... --archive-gate` completion condition).
