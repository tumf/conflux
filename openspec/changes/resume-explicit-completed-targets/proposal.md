...
The repository already has `is_merged_to_base()`, which verifies an archive entry in the base-branch tree and absence of the active change directory without relying on commit messages. It also has managed worktree discovery. These facts can classify explicit targets before dispatch without introducing durable external state.
...
The behavior applies to explicit cumulative parallel targets whether or not `-u` is enabled. `--all`, serial mode, dry-run side-effect suppression, duplicate rejection, dependency selection, and existing phase detection remain unchanged except for reporting already-completed/resumable classifications.
...
2. Base completion is proven by repository tree evidence: matching archive entry plus absence of the active change directory on the captured base branch; commit subject, logs, events, and server DB are not accepted as proof.
...
8. Parallel dry-run performs the same read-only classification and reports active, already-completed, resumable, and unknown results without mutating or cleaning worktrees.
9. Serial target filtering and `--all` behavior remain unchanged; `-u` consumes the same classification rather than adding a server-specific path.
...
- `Orchestrator` resolves explicit parallel targets only after capturing the attached base branch and before any workspace mutation.
- Base-integrated classification reuses or factors the existing `is_merged_to_base()` tree contract and tests exact/date-prefixed archives, active-directory contradiction, missing branch, and command failure.
...
- Real Git/worktree tests repeat identical target sets across active, archived+integrated, archived-not-integrated worktree, malformed worktree, unknown, duplicate, dry-run, and `--no-resume` cases.
