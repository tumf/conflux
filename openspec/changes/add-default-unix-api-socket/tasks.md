...
- Attempt 1 remediation evidence cited two test locations that had shifted after the fixes landed. The reconciled references are `tests/run_exit_tests.rs:1228` (`a_tui_bind_failure_refuses_before_the_upstream_fetch`) and `src/web/listener_tests.rs:312` (`a_failed_tcp_bind_publishes_nothing_and_stops_the_unix_listener`), verified against the committed tree; the attempt-1 evidence lines that carried the stale `:1207`/`:310` numbers were superseded when the follow-up section rolled to attempt 2.

## Current Acceptance Follow-up
- attempt: 2
- [x] [acceptance-worktree-dirty] (minor) Worktree is not clean, so acceptance cannot hand off to archive | evidence: git status --short reports: M openspec/changes/add-default-unix-api-socket/tasks.md; The uncommitted diff updates two acceptance evidence line references: tests/run_exit_tests.rs:1207→1228 and src/web/listener_tests.rs:310→312 | required_changes: openspec/changes/add-default-unix-api-socket/tasks.md — Reconcile and commit the intentional line-reference corrections so the worktree is clean before acceptance reruns | verification: openspec/changes/add-default-unix-api-socket/tasks.md — Verify git status --short is empty, then rerun the archive gate
  finding: {"evidence":["git status --short reports: M openspec/changes/add-default-unix-api-socket/tasks.md","The uncommitted diff updates two acceptance evidence line references: tests/run_exit_tests.rs:1207→1228 and src/web/listener_tests.rs:310→312"],"id":"acceptance-worktree-dirty","required_changes":[{"description":"Reconcile and commit the intentional line-reference corrections so the worktree is clean before acceptance reruns","file":"openspec/changes/add-default-unix-api-socket/tasks.md"}],"severity":"minor","summary":"Worktree is not clean, so acceptance cannot hand off to archive","verification":[{"description":"Verify git status --short is empty, then rerun the archive gate","file":"openspec/changes/add-default-unix-api-socket/tasks.md"}]}
  evidence: The corrected references are reconciled in the new "Acceptance Repair Notes" section of this file and re-verified by grep: `tests/run_exit_tests.rs:1228` and `src/web/listener_tests.rs:312` are the current definition lines of the two named tests.
  evidence: All tasks.md edits are committed, so `git status --short` in this worktree prints nothing.
  evidence: `cflx openspec validate add-default-unix-api-socket --archive-gate` rerun on the clean committed tree.
