...
- [x] Preserve legitimate manual deferral behavior for real dirty-base blockers. (verification: unit - keep or add a test where base has uncommitted changes before merge and the result remains `MergeDeferred(auto_resumable=false)` / `MergeWait`)
