...
1. **LogEntry構造体の拡張** (`src/events.rs`)
...
- [ ] LogEntry構造体に新しいフィールドとビルダーメソッドが追加されている
- [ ] ログヘッダーが `[change_id:operation:iteration]` 形式で表示される
- [ ] オペレーション/イテレーションがない場合は従来の形式で表示される（後方互換性）
- [ ] 並列実行時にapplyログにイテレーション番号が表示される
- [ ] archive/resolveログに適切なオペレーションタイプが表示される
- [ ] 既存のテストがすべてパスする
- [ ] 新しいテストがすべてパスする
- [ ] `cargo fmt` と `cargo clippy` が警告なく完了する
