...
オーケストレーターは、逐次/並列のどちらの apply でも共通ループで同一の履歴注入ロジックを使用し、各 apply 試行の最終サマリーメッセージと stdout/stderr の末尾要約を記録して同一 change の次回 apply プロンプトに含めなければならない（MUST）。

#### Scenario: parallel の2回目 apply に履歴が含まれる
- **GIVEN** parallel mode で change が apply 実行中である
- **AND** 1回目の apply がエージェントのサマリーと stdout/stderr を返している
- **WHEN** 2回目の apply が実行される
- **THEN** プロンプトは base apply_prompt を含む
- **AND** プロンプトは `<last_apply attempt="1">` ブロックを含む
- **AND** ブロックには 1回目のサマリーと stdout/stderr の末尾要約が含まれる
...
オーケストレータは、逐次/並列のどちらの archive でも共通ループで同一の履歴注入ロジックを使用し、各 archive 試行の結果と stdout/stderr の末尾要約をキャプチャして同じ change に対する後続の archive プロンプトに含めなければならない（MUST）。
...
- **AND** ブロックには試行回数、成功/失敗ステータス、所要時間、検証結果、stdout/stderr の末尾要約が含まれる
...
- **AND** 各ブロックにはそれぞれの試行の詳細と stdout/stderr の末尾要約が含まれる
...
- **AND** ブロックには 1回目の試行結果と stdout/stderr の末尾要約が含まれる

## ADDED Requirements
### Requirement: Resolve Context History Output

オーケストレータは、各 resolve 試行の結果と stdout/stderr の末尾要約をキャプチャして、同一セッション内の後続 resolve プロンプトに含めなければならない（MUST）。

#### Scenario: 2回目の resolve には前回の試行結果と出力が含まれる
- **GIVEN** conflict resolution の1回目の試行が検証失敗した
- **WHEN** オーケストレータが同じセッションで2回目の resolve を実行する
- **THEN** プロンプトには `<resolve_context>` ブロックが含まれる
- **AND** ブロックには前回の試行の command exit、verification 結果、stdout/stderr の末尾要約が含まれる
