...
### ~~Requirement: Workspace Refresh After Merge~~

~~マージ後の working copy リフレッシュ要件を削除。~~ 

この要件は `workspace update-stale` の使用を前提としていたため、削除されました。代わりに、`jj new` による新しい working copy コミット作成で working copy を管理します。
