...
- [ ] 3. Update proposal chat message reconciliation so reload/reconnect cannot append duplicate logical assistant turns when history hydration and replay both occur (verification: frontend tests cover reload/reconnect and assert no duplicate assistant messages for one logical turn).
...
- [ ] 5. Add regression coverage for cross-session isolation and idempotent history restoration across REST load, reconnect replay, and browser reload (verification: proposal chat hook/component tests plus backend/integration tests cover duplicate-prevention and session-isolation scenarios).
