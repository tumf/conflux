...
- [ ] **Task 3: Move TUI start, mark, queue add/remove, and retry handling onto the shared service** without changing key bindings or frontend rendering ownership. (verification: integration - adapter parity cases invoke TUI and direct service paths in `cargo test --lib operator_command -- --list | grep -q operator_command && cargo test --lib operator_command`; verification-id: operator-command-local)
