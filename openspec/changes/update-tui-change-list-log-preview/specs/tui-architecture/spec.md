...
TUI の変更一覧は、各 change の最新ログエントリを右側の空きスペースに単一行のプレビューとして表示しなければならない (MUST)。プレビューにはログの相対時刻（例: `2m`, `30s`）と短縮ヘッダ形式 `[operation:{iteration}]` または `[operation]`、およびメッセージが含まれ、表示幅に収まるように折り返しなしで省略されなければならない。

#### Scenario: 変更一覧に最新ログの相対時刻付きプレビューが表示される
- **GIVEN** ある change に 2分前のログエントリ（`operation="resolve"`、`iteration=1`）が存在する
- **WHEN** TUI が変更一覧を描画する
- **THEN** change 行に `2m [resolve:1]` と最新ログメッセージが同じ行で表示される
