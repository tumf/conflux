...
加えて、完了済みの `replace-cflx-py-with-native-cli` は skill-local Python helper 依存を native CLI へ移しており、bundled skill 配布物からも `scripts/cflx.py` を除去している。その前提では、新しく追加する operation-specific skills だけでなく `cflx-workflow` compatibility router も `cflx.py` を前提にせず、`cflx openspec ...` と self-contained な skill guidance で互換 surface を維持する必要がある。

一方で既存環境や既存 prompt 互換性のため、`cflx-workflow` 自体はすぐに削除せず、legacy compatibility router として残す必要がある。
...
- 新規 operation-specific skills と `cflx-workflow` compatibility router は `replace-cflx-py-with-native-cli` の native CLI 方針に合わせて `cflx.py` を同梱せず、必要な OpenSpec 操作は `cflx openspec ...` 前提で記述する
...
- 埋め込みスキル登録と `install-skills` 配布対象を新しい個別スキル群込みへ更新し、bundled skill 配布物に `scripts/cflx.py` を再導入しない
...
- Conflux に埋め込まれる bundled skills と `cflx install-skills` の配布結果に、新しい operation 別スキル群が含まれ、各 skill は自分に必要な auxiliary references を同梱し、`scripts/cflx.py` は bundled skill 配布物へ再導入されない
