...
1. establish current behavior and root cause from repository evidence;
...
- The policy appears before detailed proposal-format instructions.
...
- `skills/cflx-proposal/SKILL.md` contains the policy near its entry guidance.
- `tests/install_skills_test.rs` verifies the policy's decisive behavioral markers.
...
- Retaining or redesigning the bundled `cflx-run` skill.
