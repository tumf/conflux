...
- [ ] `src/tui/state/events.rs`の`OrchestratorEvent::Stopped`ハンドラを修正
  - Processing/Archiving状態の変更をQueuedに戻すロジックを追加
  - 中断された変更の経過時間を記録
  - Force stopの実装を参考にする

- [ ] テストケースを追加：`test_stopped_event_cleans_up_processing_changes`
  - Processing状態の変更がQueuedに戻ることを確認
  - Archiving状態の変更もQueuedに戻ることを確認
  - 複数の変更が同時にProcessing/Archivingの場合も正しく処理されることを確認

- [ ] テストケースを追加：`test_stopped_event_records_elapsed_time`
...
- [ ] 手動テストシナリオ2：Archiving中のGraceful stop
  - 変更の処理が完了し、Archiving状態になるまで待つ
  - Escキーを押してGraceful stopを実行
  - Archiving状態の変更もQueuedに戻ることを確認

- [ ] 手動テストシナリオ3：Force stopとの比較
