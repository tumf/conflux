...
- [x] `cflx server` がグローバル `server.bind` / `server.port` を実質的に無視しているため、CLI で未指定の値のみグローバル設定を採用するように修正する（実装: `src/cli.rs` の `ServerArgs.bind` を `String` から `Option<String>` に、`port` を `u16` から `Option<u16>` に変更し、デフォルト値の指定を削除。`src/main.rs` では `args.bind.as_deref()` と `args.port` をそのまま渡すことで、CLI未指定時は `None` が渡されてグローバル設定の値が保持される）。
- [x] `POST /api/v1/projects/{id}/git/pull` が non-fast-forward 失敗時に `non_fast_forward` 理由を返すように修正し、bare repo を使った再現テストを追加する（実装: `src/server/api.rs` の `git_pull` で fetch 前にローカルブランチのHEADを記録し、fetch後にリモートのSHAと比較。ローカルがリモートの祖先でない場合（diverged）に `error: "non_fast_forward"` を含む422レスポンスを返す。`test_git_pull_non_fast_forward_detection` テストで bare repo を用いた分岐状態を再現して検証）。
