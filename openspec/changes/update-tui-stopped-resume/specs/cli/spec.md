...
TUIはStoppedモードを提供し、実行中のみqueuedを保持する方針で変更状態を管理しなければならない（SHALL）。Stopped遷移時はqueue_statusをNotQueuedへ戻し、実行マーク（[x]）は保持する。Stoppedモード中のSpace操作は実行マークの付与/解除のみを行い、queue_statusはNotQueuedのまま維持する。F5再開時は実行マークの付いたchangeをqueuedに復元して処理を再開する。Stopped中のタスク進捗更新はqueued化を行わない。
