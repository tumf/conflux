...
- [x] Archive commit path remains blocked: `cflx openspec validate persist-acceptance-stalled-state --archive-gate` exits 1. `openspec/changes/persist-acceptance-stalled-state/tasks.md:20` is still a self-referential final-validation checkbox, and line 21 is a behavior-bearing checked task without `(verification: ...)`. Move final validation out of the checkbox section and add verification evidence to the serial restoration follow-up.
- [x] Parallel restart restoration remains incomplete: `src/parallel/dispatch.rs:991-997` restores `cycle_count` and `previous_finding_identities` through `set_follow_up_findings` but discards checkpoint `semantic_fingerprint`. This does not reconstruct the full semantic baseline required by `openspec/changes/persist-acceptance-stalled-state/specs/parallel-execution/spec.md:5-14`. Restore with `AcceptanceHistory::set_checkpoint` and add a parallel restart assertion for the semantic fingerprint.
