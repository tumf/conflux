...
verifications:
  - id: proposal-gate-validation-tests
    ...
    automation: src/openspec_cmd.rs
...
- A change-blocking declaration whose rerun command contains a configurable denylist of structurally heavyweight execution forms is rejected. The initial native list covers Docker/container orchestration, cross-architecture emulation, benchmark commands, and explicit full/exhaustive/heavy suite selectors.
- Repetition or stability loops remain invalid Apply-blocking work.
...
- Strict validation rejects structurally identified heavyweight commands as change-blocking evidence and recommends bounded local proof plus non-blocking broad verification.
...
`src/openspec_cmd.rs` owns bounded repository-local validator regression coverage. Repository-wide checks remain owned by existing hooks.
