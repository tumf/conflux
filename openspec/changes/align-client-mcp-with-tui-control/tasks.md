...
- attempt: 1
- [x] [acceptance-top-level-help-retired-enqueue] (minor) Top-level cflx --help still advertises the retired enqueue verb in the client subcommand synopsis | evidence: src/cli.rs:43 long_about reads `client   Operate an existing owner (status/enqueue/wait/mcp) without becoming one` and the built binary prints it verbatim in `cflx --help`; `cflx client enqueue` no longer exists: ClientCommands (src/cli.rs:1066-1197) has variants Status/Mark/Unmark/Start/Stop/ForceStop/Wait/Subscribe/Mcp only; Proposal explicit completion condition requires help to describe explicit control and subscription only, and the documentation delta requirement 'Client documentation teaches explicit TUI-equivalent control and subscription' forbids recommending cflx_enqueue in CLI help; tests/client_cli_tests.rs:165 client_control_help_documents_operator_verbs_and_retires_enqueue asserts only `cflx client --help`, so the top-level `cflx --help` synopsis is uncovered | required_changes: src/cli.rs — Update the top-level long_about client synopsis at src/cli.rs:43 to name only current client verbs, e.g. (status/mark/start/stop/wait/subscribe/mcp), removing enqueue | verification: tests/client_cli_tests.rs — Extend client_control_help_documents_operator_verbs_and_retires_enqueue (or add a sibling test) to run top-level `cflx --help` and assert the client synopsis names current verbs and the output contains no enqueue
  finding: {"evidence":["src/cli.rs:43 long_about reads `client   Operate an existing owner (status/enqueue/wait/mcp) without becoming one` and the built binary prints it verbatim in `cflx --help`","`cflx client enqueue` no longer exists: ClientCommands (src/cli.rs:1066-1197) has variants Status/Mark/Unmark/Start/Stop/ForceStop/Wait/Subscribe/Mcp only","Proposal explicit completion condition requires help to describe explicit control and subscription only, and the documentation delta requirement 'Client documentation teaches explicit TUI-equivalent control and subscription' forbids recommending cflx_enqueue in CLI help","tests/client_cli_tests.rs:165 client_control_help_documents_operator_verbs_and_retires_enqueue asserts only `cflx client --help`, so the top-level `cflx --help` synopsis is uncovered"],"id":"acceptance-top-level-help-retired-enqueue","required_changes":[{"description":"Update the top-level long_about client synopsis at src/cli.rs:43 to name only current client verbs, e.g. (status/mark/start/stop/wait/subscribe/mcp), removing enqueue","file":"src/cli.rs"}],"severity":"minor","summary":"Top-level cflx --help still advertises the retired enqueue verb in the client subcommand synopsis","verification":[{"description":"Extend client_control_help_documents_operator_verbs_and_retires_enqueue (or add a sibling test) to run top-level `cflx --help` and assert the client synopsis names current verbs and the output contains no enqueue","file":"tests/client_cli_tests.rs"}]}
  evidence: src/cli.rs:43 synopsis now reads `client   Operate an existing owner (status/mark/start/stop/wait/subscribe/mcp) without becoming one`; `./target/debug/cflx --help | grep -c enqueue` prints 0, and tests/client_cli_tests.rs top_level_help_synopsis_names_current_client_verbs_and_retires_enqueue asserts both facts (`cargo test --test client_cli_tests` — 75 passed).
