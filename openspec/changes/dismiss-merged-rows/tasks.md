...
- [ ] Add typed process-local dismissed-merged ID state and individual/bulk confirmation payloads to `AppState`/`ModalState`, including status recheck, cursor repair, known-ID/NEW cleanup, and selected-log-filter cleanup on confirmation (verification: unit - `cargo test tui:: --lib`; verification-id: merged-row-dismissal-tests)
...
- [ ] Route `d`, `D`, case-insensitive `y`/`n`, and `Esc` for Changes-view dismissal while preserving Worktrees-view deletion and modal input ownership (verification: integration - `cargo test tui:: --lib`; verification-id: merged-row-dismissal-tests)
- [ ] Render context-aware individual/bulk hints and both confirmation overlays in the existing production TUI visual language, with deterministic tests for focused/non-focused, empty, cancel, and confirmation states (verification: integration - `cargo test tui:: --lib`; verification-id: merged-row-dismissal-tests)
