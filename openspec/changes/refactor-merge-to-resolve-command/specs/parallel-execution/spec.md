...
各マージコミットメッセージは `Merge change: <change_id>` の形式とし、対象ブランチに対応する `change_id` を含めなければならない（MUST）。

#### Scenario: Merge single branch

- **WHEN** 1つのワークスペースブランチをマージする
- **THEN** システムは `resolve_command` を実行し、ターゲットブランチ上で当該ブランチのマージコミットが作成される
- **AND** マージコミットメッセージは `Merge change: <change_id>` の形式である
...
- **AND** 各ブランチのマージコミットメッセージは `Merge change: <change_id>` の形式である
...
- **THEN** `resolve_command` に渡されるプロンプトに以下が含まれる:
  ...
  - マージコミットメッセージは `Merge change: <change_id>` を使用する規約
