...
- [ ] Add TUI regressions for the real bounded-exhaustion event path proving that `ResolveFailed` leaves `warning_popup` absent, preserves `Running` with unrelated active work, permits the existing `Select` transition when none remains, and leaves the affected row retryable as `merge wait`. Completion requires assertions that would fail if the handler opened an overlay, entered Error, or converted the row to terminal `error`. (verification: integration - `cargo test --lib tui::state::event_handlers::errors::tests`; verification-id: merge-wait-notification-tests)
