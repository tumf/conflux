...
The TUI MUST consume all key input while a warning or interaction modal is active and MUST NOT route `x`, Space, or any other ordinary command to the underlying view. Post-archive rows (`archived`, `merged`, and `pushed`) MUST remain outside mark controls and Space on them MUST be a silent no-op.
...
- **AND** the target is a visible pre-archive row
...
- **GIVEN** a visible pre-archive change is active or carries active Apply iteration-limit evidence
...
#### Scenario: post-archive row ignores mark input
