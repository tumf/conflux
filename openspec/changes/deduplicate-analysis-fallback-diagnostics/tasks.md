...
- [ ] Move recoverable fallback tracing and runtime warning emission behind the same existing `DiagnosticDeduplicationKey::AnalysisFailure` decision in `src/parallel_run_service.rs` (verification: unit - extend `test_recoverable_analysis_fallback_diagnostic_dedupes_by_signature` in `src/parallel_run_service.rs` and run `cargo test recoverable_analysis_fallback_diagnostic_dedupes_by_signature` to prove two equivalent emissions yield exactly one WARN tracing record and one warning runtime event).
