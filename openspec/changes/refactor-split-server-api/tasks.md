...
- [ ] `Acceptance #1 Failure Follow-up` と実装タスク 13 の完了状態を実装実態に合わせて修正し、`src/server/api/mod.rs` に API 別テストが残っている間はテスト移管完了を `[x]` のまま残さない
- [ ] `server::api::tests::test_list_worktrees_with_real_project` が再び通るよう修正し、`cargo test` を再実行して全体成功を確認する
- [ ] archive 前品質ゲートとして `cargo fmt --check` / `cargo clippy -- -D warnings` / `cargo test` / `prek run --all-files` を再実行し、すべて成功した証拠を確認する
