...
6. activate an infallible prepared scheduler permit or issue its wake after that outcome dispatch and still under the held transaction gate — so later scheduler progress produces separate events and revisions, and no command is admitted between the accepted run decision and the activation it authorized — then release the gate;
