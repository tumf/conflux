...
- attempt: 3
- [x] [dynamic-queue-intent-fail-open] (major) Already-added candidates can still dispatch after reducer queue intent is revoked | evidence: src/parallel/queue_state.rs:2381-2448 classifies every scheduler-local candidate without requiring membership in the current reducer_queued set; src/parallel/queue_state.rs:2451-2467 consults reducer_queued only for reducer IDs absent from the local candidate list; src/parallel/tests/executor.rs:12539-12622 discards the already-added candidate by using fresh vectors after revocation, so it does not exercise classification or dispatch of a stale local candidate; src/parallel/tests/executor.rs:12848-12905 covers unreadable reducer state and MergeWait, not a readable reducer whose ordinary queue intent was revoked | required_changes: src/parallel/queue_state.rs — Require current reducer-owned ordinary eligibility before classifying scheduler-local ordinary candidates as dispatchable, while preserving explicit initial-target and reducer-owned lane semantics.; src/parallel/tests/executor.rs — Add a regression that keeps an already-added candidate locally, revokes it with RemoveFromQueue or DequeueChange under a readable reducer, and proves it cannot reach analysis or dispatch until explicit requeue. | verification: src/parallel/tests/executor.rs — Assert readable reducer classification makes the already-added revoked candidate non-dispatchable, then assert AddToQueue restores eligibility.
  finding: {"evidence":["src/parallel/queue_state.rs:2381-2448 classifies every scheduler-local candidate without requiring membership in the current reducer_queued set","src/parallel/queue_state.rs:2451-2467 consults reducer_queued only for reducer IDs absent from the local candidate list","src/parallel/tests/executor.rs:12539-12622 discards the already-added candidate by using fresh vectors after revocation, so it does not exercise classification or dispatch of a stale local candidate","src/parallel/tests/executor.rs:12848-12905 covers unreadable reducer state and MergeWait, not a readable reducer whose ordinary queue intent was revoked"],"id":"dynamic-queue-intent-fail-open","required_changes":[{"description":"Require current reducer-owned ordinary eligibility before classifying scheduler-local ordinary candidates as dispatchable, while preserving explicit initial-target and reducer-owned lane semantics.","file":"src/parallel/queue_state.rs"},{"description":"Add a regression that keeps an already-added candidate locally, revokes it with RemoveFromQueue or DequeueChange under a readable reducer, and proves it cannot reach analysis or dispatch until explicit requeue.","file":"src/parallel/tests/executor.rs"}],"severity":"major","summary":"Already-added candidates can still dispatch after reducer queue intent is revoked","verification":[{"description":"Assert readable reducer classification makes the already-added revoked candidate non-dispatchable, then assert AddToQueue restores eligibility.","file":"src/parallel/tests/executor.rs"}]}
  evidence: src/parallel/queue_state.rs classify_queued_work now withholds any scheduler-local candidate the reducer no longer admits (after the acceptance-hold, MergeWait, lane-wait and terminal-error branches), classifying it CandidateUnavailable instead of DispatchableApply
  evidence: src/parallel/queue_state.rs select_changes_for_dispatch applies the same reducer-intent gate, and reconcile_queued_candidates_from_shared_state drops revoked candidates from the local list (revoked_removed) while keeping in-flight, active and merge/resolve/reject-wait work
  evidence: src/parallel/tests/executor.rs revoked_queue_intent_stops_an_already_added_candidate_before_analysis_and_dispatch keeps the already-added candidate locally, revokes it with RemoveFromQueue and with DequeueChange under a readable reducer, and asserts no analyzer input, no dispatch selection and no dispatch until explicit requeue
  evidence: src/parallel/tests/executor.rs asserts class_for("revoked") is DispatchableApply before revocation, CandidateUnavailable after it while "keeper" stays dispatchable, and DispatchableApply plus dispatch selection again after AddToQueue
  evidence: verified end to end - cargo test --lib passes 2817 tests (0 failed) with the new regression, and cargo fmt -- --check plus cargo clippy --all-targets -- -D warnings are clean
