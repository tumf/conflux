...
- [ ] 1. `LogLevel` enumを`src/events.rs`に追加
  - `Info`, `Success`, `Warn`, `Error`の4レベル
  - `#[derive(Debug, Clone, Copy, PartialEq, Eq)]`を適用

- [ ] 2. `LogEntry`に`level`フィールドを追加
  - `pub level: LogLevel`
  - 既存の`color`フィールドは維持（TUI表示用）

- [ ] 3. `LogEntry`のコンストラクタを更新
  - `info()` → `LogLevel::Info`
  - `success()` → `LogLevel::Success`
  - `warn()` → `LogLevel::Warn`
  - `error()` → `LogLevel::Error`

- [ ] 4. `src/tui/state/logs.rs`に`tracing`のインポートを追加

- [ ] 5. `add_log()`メソッドでレベルに応じた`tracing`出力を追加
...
- [ ] 7. `cargo fmt && cargo clippy && cargo test`で検証
