...
Acceptance #1 Failure Follow-up の3件目は behavior-bearing checkbox として解釈されないよう整理済み。最終 OpenSpec 検証コマンドは自己参照 final validation checkbox にならないよう、非チェックボックスの `## Final Validation` に集約する。
...
## Acceptance #5 Resolution Notes

Acceptance #5 の archive gate 指摘は、`tasks.md:29` の Acceptance #2 Failure Follow-up が behavior-bearing checkbox と判定されていたことが原因だった。対応として Acceptance #2 の記録を非チェックボックスの Resolution Notes へ移し、最終 OpenSpec 検証コマンドは `## Final Validation` に集約する。
