...
- [x] openspec/specs/agent-prompts/spec.md:226-232 がまだ canonical wording として `blocked` を残しています。scenario title が「`blocked` 保留を要求する」、Then が「`blocked` 保留 outcome」、AND が「change を `Blocked` へ送る前提」となっており、同 requirement 本文および change delta (openspec/changes/unify-acceptance-gated-verdict/specs/agent-prompts/spec.md:29-35) の `gated` 契約と不一致です。ここは `gated hold` / `acceptance-gated` に更新し、dependency wait の `Blocked` と切り分けてください。
- [x] openspec/specs/orchestration-state/spec.md:439-446 が rejection flow scenario で依然として `acceptance confirms a blocked verdict` を使っています。前回指摘の requirement prose (:415, :435) は `Gated` / `acceptance-gated` に直っていますが、scenario の canonical wording が未更新のため、proposal の acceptance criteria (openspec/changes/unify-acceptance-gated-verdict/proposal.md:52-57) をまだ満たしていません。`blocked verdict` を `gated verdict` へ揃えてください。
