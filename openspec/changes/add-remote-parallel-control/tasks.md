## Implementation Tasks

- [x] Add parallel runtime mode, availability, maximum concurrency, VCS backend, and per-change eligibility/reason to capabilities and coherent state projection. (verification: unit - `cargo test --features web-monitoring --lib` verifies projection fixtures cover sequential/parallel, unavailable Git, eligible, uncommitted, active, and final rows; verification-id: remote-parallel-tests)

- [x] Add a revision-fenced parallel-toggle command routed through the shared operator service with Select/Stopped guards and TUI-equivalent ineligible-mark cleanup. (verification: integration - `cargo test --features web-monitoring --lib` verifies TUI/v2 parity tests compare changed marks, mode, feedback, invalid-mode no-op/failure, and idempotent replay; verification-id: remote-parallel-tests)

- [x] Add an atomic bulk execution-mark command that classifies one snapshot, chooses one target state from eligible rows, updates Running queue intent consistently, clears applicable NEW attention, and returns changed IDs plus stable exclusion reasons. (verification: integration - `cargo test --features web-monitoring --lib` verifies bulk tests cover all-mark/all-unmark, active/rejected/uncommitted exclusions, zero eligible, stale revision, and no partial effects; verification-id: remote-parallel-tests)

- [x] Fence start against the full marked eligibility set so parallel start is all-or-nothing and returns actionable target-ineligible details. (verification: integration - `cargo test --features web-monitoring --lib` verifies start tests prove one ineligible target prevents scheduler spawn and leaves all marks/queue intents coherent; verification-id: remote-parallel-tests)

- [x] Register commands, schemas, capabilities, events, and parity fixtures in the v2 API and generated OpenAPI surface. (verification: integration - `cargo test --features web-monitoring --lib` verifies remote-control route/schema tests execute success, no-op, rejection, and replay paths; verification-id: remote-parallel-tests)

## Implementation Notes

- The sequential/parallel toggle now lives in one process-local
  `ParallelRuntime` store (`src/orchestration/operator_command.rs`). The TUI
  publishes availability, concurrency, backend, and per-change eligibility into
  it and *adopts* the toggle from it; the shared start guard
  (`StartEligibility`) is the same store, so a keypress, a remote command, and
  the start fence can never read three different modes.
- `=` in the TUI now emits `TuiCommand::SetParallelMode`, which calls the same
  `OperatorCommandService::set_parallel_mode` the `/api/v2` command calls. Bulk
  marking shares `plan_bulk_marks`, so both frontends derive one target set and
  one exclusion vocabulary.
- Start is fenced against the *complete* marked set before it is narrowed to
  startable rows, which is what makes parallel start all-or-nothing.
- The snapshot's per-change `actions` block consults parallel eligibility with
  the same precedence `classify_bulk_mark_row` uses, so a row parallel mode
  would refuse to start is never advertised as markable.
- `set_all_execution_marks` is a struct variant with `deny_unknown_fields`, so a
  client-supplied `marked` is a schema failure rather than an ignored field.
- `docs/openapi.yaml` was regenerated (`make openapi`) for the new commands,
  capabilities, and parallel schemas.
- The toggle and the bulk mark are each one indivisible mutation: both take the
  shared `ParallelRuntime::lock_mutations()` guard for their whole body, and the
  toggle clears the intent parallel mode refuses before it publishes the mode.
  Without that, two commands admitted concurrently by `/api/v2/commands` (each
  runs on its own task) could interleave — a bulk mark that classified in
  sequential mode resuming after the cleanup and re-marking the very row the
  cleanup had just cleared.

## Final Validation

Archive validation itself is the authoritative final OpenSpec validation gate.
Expected archive gate: `cflx openspec validate add-remote-parallel-control --archive-gate`

The implementation must also pass `cargo test --features web-monitoring --lib`.

## Future Work

- Scheduler optimization and removal of obsolete serial mode remain separate changes.

## Current Acceptance Follow-up
- attempt: 1
- [x] [remote-parallel-concurrent-mutation-race] (major) Concurrent parallel toggle and bulk-mark commands can leave an ineligible change marked in parallel mode | evidence: src/web/remote_control_api/commands.rs:151 spawns every admitted command independently without serializing shared operator mutations; src/orchestration/operator_command.rs:1130 enables parallel mode before cleanup and then awaits queue operations; src/orchestration/operator_command.rs:1186 captures parallel_mode before awaiting the reducer snapshot, allowing a concurrently enabled mode to be missed; A bulk command that captured sequential mode can resume after toggle cleanup and mark an ineligible row, violating atomic coherent intent requirements | required_changes: src/orchestration/operator_command.rs — Serialize parallel-mode changes and bulk execution-mark mutations through one shared mutation guard so classification, cleanup, and mark/queue updates cannot interleave | verification: src/web/remote_control_api/tests/parallel_control_tests.rs — Add a deterministic concurrent toggle/bulk regression test proving parallel mode never settles with marks or queue intent on ineligible rows
  finding: {"evidence":["src/web/remote_control_api/commands.rs:151 spawns every admitted command independently without serializing shared operator mutations","src/orchestration/operator_command.rs:1130 enables parallel mode before cleanup and then awaits queue operations","src/orchestration/operator_command.rs:1186 captures parallel_mode before awaiting the reducer snapshot, allowing a concurrently enabled mode to be missed","A bulk command that captured sequential mode can resume after toggle cleanup and mark an ineligible row, violating atomic coherent intent requirements"],"id":"remote-parallel-concurrent-mutation-race","required_changes":[{"description":"Serialize parallel-mode changes and bulk execution-mark mutations through one shared mutation guard so classification, cleanup, and mark/queue updates cannot interleave","file":"src/orchestration/operator_command.rs"}],"severity":"major","summary":"Concurrent parallel toggle and bulk-mark commands can leave an ineligible change marked in parallel mode","verification":[{"description":"Add a deterministic concurrent toggle/bulk regression test proving parallel mode never settles with marks or queue intent on ineligible rows","file":"src/web/remote_control_api/tests/parallel_control_tests.rs"}]}
  evidence: src/orchestration/operator_command.rs:411,514 add `ParallelRuntime::mutations` + `lock_mutations()`; set_parallel_mode (:1152) and set_all_execution_marks (:1221) hold that one guard across their whole mutation, and the toggle now clears ineligible intent *before* publishing the mode
  evidence: src/web/remote_control_api/tests/parallel_control_tests.rs:635,754,823 add a PausingQueue that parks a mutation inside its queue effect plus both interleaving regression tests; with the guard removed both fail on `parallel mode must not settle with an ineligible change marked` (marks were ["a_eligible","z_blocked"]), with it both pass and `cargo test --features web-monitoring --lib` is 2880 passed / 0 failed
