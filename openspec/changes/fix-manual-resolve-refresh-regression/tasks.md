...
- [ ] Run formatting/lint/typecheck/test commands required for touched Rust/TUI code. (verification: manual - run the repository's relevant `cargo fmt`, `cargo clippy` or configured lint/typecheck if available, plus focused cargo tests; completion condition: commands pass or any unrelated failures are documented with evidence)
