## MODIFIED Requirements

### Requirement: Acceptance prompt MUST instruct tasks.md follow-up updates on FAIL

Acceptance guidance MUST define acceptance as read-only review and MUST NOT instruct the acceptance agent to edit `tasks.md`. When acceptance returns FAIL, runtime MUST own persistence of one `## Current Acceptance Follow-up` section, replacing prior runtime-managed or numbered acceptance follow-up sections rather than accumulating attempt history.

The current section MUST represent each repository-fixable finding as one unchecked task after normalization, sorting, and deduplication. External or non-mockable blockers MUST be retained as non-checkbox metadata with their evidence and next action. Runtime MUST NOT write `ACCEPTANCE:` or `FINDINGS:` protocol markers into `tasks.md`.

#### Scenario: runtime writes one current follow-up

- **GIVEN** acceptance returns repository-fixable FAIL findings
- **WHEN** runtime persists follow-up work
- **THEN** `tasks.md` contains exactly one current runtime-managed acceptance section
- **AND** repository findings appear once as unchecked tasks
- **AND** prior numbered runtime follow-up sections are removed or replaced

#### Scenario: acceptance agent remains read-only

- **GIVEN** bundled acceptance guidance is loaded
- **WHEN** the reviewer prepares a FAIL verdict
- **THEN** it returns actionable findings without editing `tasks.md`
- **AND** runtime remains the sole follow-up writer

#### Scenario: external blocker is not converted into implementation work

- **GIVEN** a FAIL observation includes a repository defect and an external blocker
- **WHEN** runtime writes current follow-up
- **THEN** only the repository defect is an unchecked task
- **AND** the external blocker is recorded without a checkbox and with a next action

## ADDED Requirements

### Requirement: Acceptance prompt context MUST be latest-only and non-duplicative

Acceptance prompt construction MUST include current repository diff context and the latest normalized findings once. It MUST NOT replay all prior acceptance attempts. Bounded raw stdout or stderr MAY be included for CONTINUE or command diagnostics, but MUST NOT duplicate a finalized FAIL finding payload already present in the prompt.

#### Scenario: three failed attempts expose only latest findings

- **GIVEN** acceptance history contains at least three attempts
- **WHEN** the next acceptance prompt is built
- **THEN** the prompt includes the current diff and latest normalized findings
- **AND** earlier attempt findings and output are omitted
- **AND** the latest findings are not repeated through multiple context blocks

#### Scenario: command diagnostics retain bounded raw output

- **GIVEN** the previous acceptance attempt returned CONTINUE or failed without a finalized FAIL finding payload
- **WHEN** the next prompt is built
- **THEN** it may include bounded latest stdout or stderr needed for diagnosis
- **AND** it does not restore all-attempt history
