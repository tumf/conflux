...
### Requirement: ParallelEvent Type

**Reason**: 統一イベント型 `ExecutionEvent` に置き換えられるため。

**Migration**: `src/parallel/events.rs` の `ParallelEvent` enum を削除し、`ExecutionEvent` を使用する。すべての発行箇所と受信箇所を更新する。
