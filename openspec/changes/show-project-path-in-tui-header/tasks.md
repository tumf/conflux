...
- [ ] Update existing header regression assertions that currently require `[workspaces:...]` so they require a full or deterministically middle-elided project path and explicitly reject the retired badge. (verification: unit - `cargo test --locked tui::render`; verification-id: tui-project-path-header)
