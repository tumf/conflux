...
- [ ] Cargo.tomlに `local-ip-address` crateを追加（ローカルIP取得用）
- [ ] フィーチャーフラグの設定を確認
...
### 4. URL変換モジュール
- [ ] `src/web/url.rs` または該当モジュールにURL変換ロジックを追加
- [ ] `build_access_url(bind_addr, port)` 関数を実装
- [ ] `get_local_ip()` 関数を実装（`local-ip-address` crate使用）
- [ ] ユニットテストを追加
...
- [ ] `0.0.0.0` バインド時にローカルIPアドレスがURLに使用される
