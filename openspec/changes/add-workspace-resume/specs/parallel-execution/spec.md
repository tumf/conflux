...
- **AND** workspaceのパス、進捗状況、最終更新時刻が含まれる
...
- **THEN** `WorkspaceInfo` が返される
- **AND** worktreeのパス、進捗状況、最終更新時刻が含まれる
...
### Requirement: Workspace Progress Check

システムは既存workspaceのタスク進捗を確認しなければならない（SHALL）。

#### Scenario: 進捗ありのworkspace

- **WHEN** 既存workspaceが検出される
- **AND** tasks.mdファイルが存在する
- **AND** 完了タスクが1つ以上ある
- **THEN** `WorkspaceInfo.progress` に進捗情報が設定される
- **AND** 再利用推奨として判断される

#### Scenario: 進捗なしのworkspace

- **WHEN** 既存workspaceが検出される
- **AND** tasks.mdの完了タスクが0である
- **AND** 未コミットの変更がない
- **THEN** 再利用のメリットが低いとして判断される
- **AND** 確認プロンプトでその旨が表示される

#### Scenario: 未コミット変更ありのworkspace

- **WHEN** 既存workspaceが検出される
- **AND** VCSで未コミットの変更がある
- **THEN** `WorkspaceInfo.has_uncommitted_changes` が `true` に設定される
- **AND** 再利用推奨として判断される
