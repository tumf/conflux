...
- [ ] 1. startup log の canonical requirement を `openspec/changes/log-startup-version/specs/observability/spec.md` と `openspec/changes/log-startup-version/specs/cli/spec.md` に追加し、TUI / run / server 起動時に version/build と mode が記録されることを定義する (verification: integration - `cflx openspec validate log-startup-version --strict --evidence warn` が成功し、`openspec/changes/log-startup-version/specs/observability/spec.md` と `openspec/changes/log-startup-version/specs/cli/spec.md` に TUI / run / server の versioned startup scenario が存在する)
- [ ] 2. `src/main.rs` の TUI / run / server 起動入口を更新し、logging 初期化直後または orchestration 開始直前に version/build と mode を含む startup log を一貫した wording で出す (verification: unit/integration - `src/main.rs` に TUI / run / server の 3 経路から共通 startup log helper または同等の versioned log 呼び出しが存在し、対応する tests またはレビュー証跡で mode ごとの message 構築が確認できる)
- [ ] 3. `src/cli.rs`、`src/tui/utils.rs`、または共通 helper を整理し、CLI `--version`、TUI 表示、startup log が semver/build の同一ソースを共有して表記ゆれを起こさないようにする (verification: unit - `src/tui/utils.rs` の `test_get_version_string` と、必要なら `src/cli.rs` 近傍の追加 test で startup log 用の version/build 表記が CLI/TUI と同一フォーマットであることを確認する)
