...
- [ ] 4. 起動ログの典型経路を検証し、`cflx` / `cflx run` / `cflx server` の startup log から version/build と mode が読み取れることを確認する (verification: manual - `cargo run -- run --max-iterations 1` の stdout で `Starting cflx v... mode=run` を確認し、`cargo run -- server --bind 127.0.0.1 --port 39899` の stdout で `Starting cflx v... mode=server` を確認済み。`cargo run` (default TUI) は非TTY環境で `ratatui` 初期化 panic (`Device not configured`) となるため、`mode=tui` の最終確認は実TTY環境で実施が必要)
...
- default TUI (`cflx` / `cflx tui`) の startup log (`mode=tui`) を実TTY環境で manual 検証する
