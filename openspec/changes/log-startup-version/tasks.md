...
- [x] 4. 起動ログの典型経路を検証し、`cflx` / `cflx run` / `cflx server` の startup log から version/build と mode が読み取れることを確認する (verification: manual - `agent-exec run -- cargo run -- run --max-iterations 1` (job: e0cefd6647a32acd435f9e82860aeb9d) の stdout で `Starting cflx v... mode=run` を確認し、`agent-exec run -- cargo run -- server --bind 127.0.0.1 --port 39899` (job: 7975635ad022110ba08a41f7a384274e) の stdout で `Starting cflx v... mode=server` を確認済み。`cargo run` (default TUI) は非TTY環境で `ratatui` 初期化 panic (`Device not configured`) となるため、`mode=tui` の最終確認は実TTY環境で実施が必要)
