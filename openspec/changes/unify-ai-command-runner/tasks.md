...
- [ ] 1.1 `src/ai_command_runner.rs` モジュールを新規作成
...
- [ ] 1.2 `execute_streaming()` メソッドを実装
  - `CommandQueue::execute_with_retry_streaming()` を内部で呼び出す
  - stdout/stderr の streaming を mpsc channel で返す
  - cwd オプションをサポート（worktree 用）
- [ ] 1.3 `src/main.rs` に `mod ai_command_runner;` を追加

## 2. AgentRunner の共有状態対応

- [ ] 2.1 `AgentRunner::new_with_shared_state()` を追加
  - 既存の `new()` は維持（後方互換性）
  - 共有状態を受け取り、内部の `CommandQueue` に渡す
- [ ] 2.2 `CommandQueue` に共有状態注入機能を追加
...
- [ ] 3.1 `ParallelExecutor` に共有 `AiCommandRunner` を追加
...
- [ ] 5.1 `src/analyzer.rs` に `validate_json_schema()` 関数を追加
  - `serde_json::from_str::<Value>()` でパース
  - `groups` キーの存在確認
  - `groups` が配列であることを確認
- [ ] 5.2 `parse_response()` で strict validation を適用
