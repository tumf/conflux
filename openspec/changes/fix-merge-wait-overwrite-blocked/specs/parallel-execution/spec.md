...
TUI の定期リフレッシュにおける `apply_merge_wait_status` は、`Blocked` 状態の変更を `MergeWait` に遷移させてはならない（MUST NOT）。これにより、依存関係による実行保留状態が TUI リフレッシュで誤って解除されることを防ぐ。
...
#### Scenario: Blocked 状態の変更が TUI リフレッシュで MergeWait に上書きされない
- **GIVEN** 変更 B が依存関係により `Blocked` 状態である
- **AND** 変更 B に対応するワークツリーが `WorkspaceState::Archived` である
- **WHEN** TUI の定期リフレッシュが `apply_merge_wait_status` を実行する
- **THEN** 変更 B のステータスは `Blocked` のまま維持される
- **AND** `MergeWait` に遷移しない
