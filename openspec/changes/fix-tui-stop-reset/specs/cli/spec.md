...
When resuming from Stopped mode, the TUI SHALL reset any previous stop/cancel state and start processing using the current execution mode (serial/parallel).
...
- **AND** stop/cancel state from previous runs is cleared before starting
- **AND** processing starts using the current execution mode (serial/parallel)
