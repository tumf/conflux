...
- [x] 1. `.pre-commit-config.yaml` の fix-up hook 設定を更新し、`dashboard/dist/assets/index-*.(js|css)` など publish 対象の committed generated assets が `end-of-file-fixer` で書き換えられないようにする (verification: integration - update `.pre-commit-config.yaml`, run `bash dashboard/build.sh`, then run `pre-commit run --all-files` or `prek run --all-files`, and confirm `dashboard/dist/assets/index-*` files are not modified by fixer hooks)
