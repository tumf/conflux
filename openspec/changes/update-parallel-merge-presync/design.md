...
## Decisions
- Decision: 逐次マージの前に「base → worktree」の事前同期を行う
  - Why: コンフリクト解消をworktree側に閉じ込めることで、統合先ブランチの状態を安定させる
...
## Open Questions
- 事前同期を常時有効（デフォルト）にするか、設定で切り替えるか
- 事前同期フェーズのマージコミット（もし発生する場合）に、メッセージ規約を設けるべきか
