...
acceptance ループは change に対して `acceptance_command` を実行し、出力テキストから pass/fail/continue と acceptance blocker compatibility verdict（`gated` / legacy `blocked`）を判定して処理を分岐しなければならない（SHALL）。

移行期間中は、旧 integration 互換のため legacy `blocked` acceptance verdict input を受理してもよい（MAY）。ただし operator-facing lifecycle wording は `gated` ではなく `stalled` を用いなければならない（MUST）。

#### Scenario: CLI maps gated compatibility verdict to stalled lifecycle hold
...
- **AND** operator-facing lifecycle status は `stalled` として表示される
...
- **THEN** change は acceptance gate outcome として処理される
- **AND** operator-facing lifecycle status は `stalled` のまま維持される
