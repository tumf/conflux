...
TUI は反復回数がある状態の表示を `status:iteration` 形式にしなければならない（SHALL）。apply/acceptance/archive/resolve の iteration 番号が更新された場合、Changes 一覧のステータス表示は最新の iteration に同期し続けなければならない（SHALL）。
