...
git-sync は、サーバーモードにおいてローカル bare リポジトリとリモートリポジトリ間のブランチ同期を管理する。pull フェーズでリモート状態を取得した後、push 前の reconciliation（`resolve_command` による AI エージェント起動）が必要かどうかを SHA 比較で事前判断し、不要な高コスト処理を回避する。
